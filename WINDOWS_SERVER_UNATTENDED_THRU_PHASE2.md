...
- **`windows_version=2025` diverges from the above in exactly two small, isolated ways** (see [Finding 15](#finding-15-windows-server-2025-media-reliably-fails-the-uefi-boot-key-mechanism-entirely--a-known-unresolved-upstream-issue-resolved-2026-09-05-by-routing-around-it--see-the-end-of-this-finding)'s resolution): `build.sh` patches a `_noprompt` variant of the cached stock ISO before Packer ever sees it (no "press any key" prompt on the media, so `boot_command` is empty instead of spamming spacebar), and `packer/locals.pkr.hcl`'s `"2025"` profile adds `qemuargs = [["-cpu", "host"]]`. Everything else — disk/network device model, driver delivery mechanism, answer-file structure — is identical to 2022's.
...
### Finding 15: Windows Server 2025 media reliably fails the UEFI boot-key mechanism entirely — a known, unresolved upstream issue (RESOLVED 2026-09-05 by routing around it — see the end of this finding)
...
**RESOLVED (2026-09-05).** The upstream bug itself is still open and unfixed (see the 2026-09-05 re-research update above) — this was unblocked by routing around it, not by anyone fixing Packer/QEMU/OVMF. Two independent things had to be found, in this order:

**Fix 1 — the boot loop itself: swap the ISO's boot files, then supply the one CPU flag Setup actually needs.** The sibling `windows-auto-build-pipeline` project, in the course of its own unrelated Windows 11 investigation, tried the exact "adjacent technique" flagged as unverified two paragraphs up: swap `efi/microsoft/boot/{cdboot.efi,efisys.bin}` for their `_noprompt` counterparts, which Microsoft ships on the stock eval ISO itself (no ADK needed) and which skip the "press any key" prompt entirely rather than trying to time it. Run against Server 2025 media via a hand-rolled `qemu-system-x86_64` invocation (bypassing Packer's builder entirely) as a research spike for this project's benefit, it got Setup's real GUI on the first attempt — something neither project had achieved before. Reproducing this through Packer's own native QEMU builder (`build.sh` now patches a `_noprompt` variant of the cached stock ISO before ever handing it to Packer — see `build_noprompt_iso`/`prepare_noprompt_iso` there, cached under `../iso_cache/derived/`) got further than ever before but still looped, in a new way: the VM reset back to the OVMF/TianoCore splash on a real, repeating cycle. The missing piece, confirmed by a standalone repro outside Packer entirely: **`-cpu host`**. Packer's own default (no `-cpu` flag, so plain `qemu64`) doesn't expose whatever CPU feature flags 2025's Setup checks for (plausibly VBS/virtualization-related — not TPM emulation, which nothing here configures) that 2022's Setup never checks. Once added — `packer/locals.pkr.hcl`'s `"2025"` profile now sets `qemuargs = [["-cpu", "host"]]`, confirmed safe because `qemuargs` only overrides switches it names, and `-cpu` isn't one Packer generates on its own (see Finding 2, and that profile entry's own comment) — Setup reached its real GUI through the ordinary Packer build path.

**Fix 2 — a second, different failure, and a real methodology mistake worth recording honestly.** With `-cpu host` in place, Setup got much further — real file-copy progress, a real reboot — then crashed with a generic Windows Setup **"The computer restarted unexpectedly or encountered an unexpected error. Windows installation cannot proceed."** dialog partway through. This looked at first like Packer's own implicit `-drive file=...,media=cdrom` CD-ROM wiring (no explicit bus assignment) being the culprit: replacing it with explicit `ide-cd`/`bus=ide.0`/`ide.1` pinning (matching the sibling project's own working device topology) made the failure disappear in a standalone repro, and a full production integration was built around that conclusion (moving CD-ROM generation out of Packer's `cd_content` into `build.sh`, hand-wiring the entire device list via `qemuargs`). **That conclusion was wrong.** The "successful" repro had also switched to a short (≤15 character) `ComputerName` in the same test as the CD-ROM change — two variables changed at once, never isolated. A follow-up test — short `ComputerName`, Packer's *original* implicit CD-ROM wiring, no pinning at all — also succeeded cleanly, with real authenticated WinRM. The actual second cause was an **oversized `ComputerName`**: this project's own ad-hoc test `vm_name`s (`win2025-gate1-boot-retry`, `win2025-gate1-boot-final`, etc.) exceeded NetBIOS's hard 15-character limit, a constraint already enforced in the sibling project's own experiment scripts (`server2025-noprompt-setup-experiment.sh`) that simply wasn't carried over here. The CD-ROM-pinning work was reverted in full — it was real, avoidable complexity (confirmed via Packer's own docs: overriding `-drive`/`-device` drops *all* of Packer's own default generation for those switches, not just the ones named) that this project doesn't actually need. What stayed: `build.sh` and `packer/variables.pkr.hcl`'s `vm_name` variable both now reject any name over 15 characters outright, for both Windows versions (this is a general NetBIOS constraint, not 2025-specific — 2022 was just never tested with a long name before).

**Net result — confirmed via two full production runs through the real `build.sh`/Packer pipeline, not a standalone script:**
- Run 1, boot-only (empty `services.yaml`, default `vm_name` `win2025-dc`): clean success, 34m10s end-to-end.
- Run 2, with `iis` selected: clean success, 1h11m end-to-end, `install-iis.ps1` verified `W3SVC` running and the default site returning HTTP 200 — the first-ever confirmed run of this project's Phase 3 role-provisioning against real Server 2025 media.

The final diff from 2022's proven mechanism is small and fully isolated to the `"2025"` profile in `packer/locals.pkr.hcl`: an empty `boot_command` (no keystroke needed — the noprompt media skips the prompt), `qemuargs = [["-cpu", "host"]]`, and `build.sh`'s `_noprompt` ISO-patching step. Nothing about 2022's own resolved configuration changed. See `packer/locals.pkr.hcl`'s `"2025"` profile entry for the fullest inline account, including exactly why the CD-ROM-pinning detour is recorded there rather than quietly removed — so the same wrong turn isn't retried without re-deriving why.
...
4. **RESOLVED — Windows Server 2025 is no longer blocked.** Finding 15 has a full resolution writeup (2026-09-05) at the end of that finding, above. Two confirmed production runs exist (boot-only, and with `iis` selected). `iso_checksum` is still `"none"` in `locals.pkr.hcl`'s `"2025"` profile — worth pinning now that builds succeed reliably enough to make that verification meaningful, but not yet done.
...
- **`vm_name`/`PKR_VAR_vm_name` over 15 characters fails the build with a generic, unhelpful Windows Setup error** (NetBIOS's hard `ComputerName` limit — see Finding 15's resolution writeup). `build.sh` and `packer/variables.pkr.hcl`'s `vm_name` variable both now reject this outright before wasting a build cycle on it, for both Windows versions. The default `win<version>-dc` is always safe.
